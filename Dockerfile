...
WORKDIR /workspace/grab-seller
COPY grab-seller/package*.json ./
...
COPY grab-seller/gateway/nginx.conf /etc/nginx/conf.d/default.conf
COPY --from=builder /workspace/grab-seller/dist /usr/share/nginx/html
EXPOSE 80
